...
serve(async (req) => {
  ...
  try {
    ...
    if (!sessionId && !externalId) throw new Error('Invoice ID or External ID is required');
    ...
    if (sessionId) {
      ...
      if (!verifyResponse.ok) {
        if (verifyResponse.status === 404) throw new Error('Invoice not found');
      ...
      if (!invoices || invoices.length === 0) throw new Error('Invoice not found for external_id');
