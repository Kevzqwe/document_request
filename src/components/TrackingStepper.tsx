import { FileText, Settings, CheckCircle, Package, Home } from 'lucide-react';
...
const TrackingStepper = ({ status }: TrackingStepperProps) => {
  return (
    <div className="flex items-start justify-between w-full px-2">
      {TRACKING_STEPS.map((step, index) => {
        const isCompleted = trackingUtils.isStepCompleted(status, index);
        const isActive = trackingUtils.isStepActive(status, index);
            ...
            <div className="flex flex-col items-center min-w-[48px] md:min-w-[60px]">
              <div
                className={cn(
                  ...
                  isActive && 'bg-primary border-primary text-primary-foreground ring-4 ring-primary/30',
